...
impl Point {
    fn new(v: Vec<i32>) -> Point {
        Point { x: v[0], y: v[1], visited: false }
    }
...
fn solve(start: &Point, points: &mut Vec<Point>, target: &Point) -> bool {
    ...
    while !que.is_empty() {
        ...
        for cand in points.iter_mut() {
            if !cand.visited && cur.can_reach(cand) {
                // println!("{}", cand);
                que.push_back(*cand);
                cand.visited = true;
            }
...
fn main() {
    ...
    for _i in 0..n {
        ...
        let ans = solve(&start, &mut points, &target);
